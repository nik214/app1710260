...
class Chat(MDScreen):
    ...
    def answer(self, text, *args):
        ...
        elif text == 'what is bitcoin price?'or text=='bitcoin price'or text=='price of bitcoin'or text=='btc' or text=='BTC'or text=='BTC price'text=='btc price':
            response = "The Bitcoin price is " + "{:.2f}$".format(float(get_cmc_data('BTC')))
            self.add_message(response, 'left', self.bot_message_color),
        elif text == 'what is ethereum price?'or text=='ethereum price'or text=='price of ethereum'or text=='eth' or text=='ETH'or text=='ETH price'text=='eth price':
            response = "The Ethereum price is " + "{:.2f}$".format(float(get_cmc_data('ETH')))
            self.add_message(response, 'left', self.bot_message_color),
        elif text == 'what is litecoin price?'or text=='litecoin price'or text=='price of litecoin'or text=='ltc' or text=='LTC'or text=='LTC price'text=='ltc price':
            response = "The Litecoin price is " + "{:.2f}$".format(float(get_cmc_data('LTC')))
            self.add_message(response, 'left', self.bot_message_color),
        elif text == 'what is xpr price?'or text=='xpr price'or text=='price of xpr'or text=='xpr' or text=='XPR'or text=='XPR price'text=='xpr price':
            response = "The XPR price is " + "{:.2f}$".format(float(get_cmc_data('XPR')))
            self.add_message(response, 'left', self.bot_message_color),

            ##cryptoccurency percentage changes for 1hour
        elif text == 'what is the change of percentage for bitcoin for 1 hour?'or text=='bitcoin change 1 hour'or text=='bitcoin change one hour'or text=='1 hour change for bitcoin'or text== 'btc 1 hour change' :
